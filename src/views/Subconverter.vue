...
const remoteConfigSample = process.env.VUE_APP_SUBCONVERTER_REMOTE_CONFIG_SAMPLE
  ...
  data() {
    return {
      ...
      options: {
        ...
        backendOptions: [
          {value: "https://sub.leftcuz.top:8443"},
    ...
    getBackendVersion() {
      this.$axios
          ...
          .then(res => {
            ...
            a ? this.$message.success(`${this.backendVersion}` + "肥羊负载均衡增强版后端，已屏蔽免费节点池（会返回403），额外支持vless+hysteria订阅转换") : b ? this.$message.success(`${this.backendVersion}`) : this.$message.success(`${this.backendVersion}`);
          })
          .catch(() => {
